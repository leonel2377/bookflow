...
export const DEMO_ACCOUNTS = {
  client: { email: "client@demo.com", password: DEMO_PASSWORD, login: "/connexion" },
  pro: { email: "pro@studio-eclat.demo", password: DEMO_PASSWORD, login: "/pro/connexion" },
  salonSlug: "studio-eclat",
} as const;
...
export async function runDemoSeed(prisma: PrismaClient) {
  ...
  const proUser = await prisma.user.upsert({
    where: { email: DEMO_ACCOUNTS.pro.email },
    update: { passwordHash },
    create: {
  ...
  const salon = await prisma.establishment.upsert({
    where: { slug: DEMO_ACCOUNTS.salonSlug },
    update: {
      ownerId: proUser.id,
      latitude: 45.764,
      longitude: 4.8357,
  ...
  for (const member of staffList) {
    for (const day of [1, 2, 3, 4, 5, 6]) {
      await prisma.staffSchedule.upsert({
        where: { id: `schedule-${member.id}-${day}` },
        update: {},
        create: {
  ...
  return { salon, accounts: DEMO_ACCOUNTS };
}
